...
sub extract_features {
	...
	my $genef = Bio::SeqFeature::Generic->new(-seq_id => $seq_id,
					       -source => $source,
					       -primary=> GENE,
					       -start  => $f->start,
					       -end    => $f->end,
	...
	for my $mRNA ( @mrnas ) {
	    ...
	    my $mrnaf = Bio::SeqFeature::Generic->new(
					       -seq_id => $seq_id,
					       -source => $source,
					       -primary=> MRNA,
					       -start  => $mRNA->start,
		...
		for my $e ( sort { $a->start * $a->strand <=> $b->start * $b->strand } 
		    ...
		    push @newcds, Bio::SeqFeature::Generic->new
			(-seq_id => $seq_id,
			 -start  => $e->start,
			 -end    => $e->end,
			 -strand => $cds->strand,
		...
		for my $exon ( sort { $a->start * $strand <=> 
		    ...
		    push @newexons, Bio::SeqFeature::Generic->new
			( -seq_id => $seq_id,
			  -start  => $exon->start,
			  -end    => $exon->end,
			  -strand => $exon_top->strand,
